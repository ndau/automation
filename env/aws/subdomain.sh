...
usage() {
    errcho "Usage"
    errcho "CLUSTER_SUBDOMAIN=cluster.ndau.tech ./cluster-subdomain.sh"
}

if [ -z "$CLUSTER_SUBDOMAIN" ]; then
    errcho "Missing CLUSTER_SUBDOMAIN."
    usage
    exit 1
fi

# Get the parent domain
PARENT_DOMAIN=$($sed "s/[^\\.]*\\.//" <<< "$CLUSTER_SUBDOMAIN")
export PARENT_DOMAIN
