...
class Node:
    ...
    def node_id(self):
        ...
        shell = f'{self.dc_str()} run --rm --no-deps tendermint show_node_id'
        cr = subprocess.run(
            shell,
            shell=True,
            stdout=subprocess.PIPE,
    ...
    def ndauhome(self, create=True):
        """
        Return this node's NDAUHOME.

        If `create`, create this directory if it doesn't already exist.
        """
        p = os.path.join(self.path(), 'ndau')
        if create:
            os.makedirs(p, exist_ok=True)
        return p

    def tmhome(self, create=True):
        """
        Return this node's TMHOME.

        If `create`, create this directory if it doesn't already exist.
        """
        p = os.path.join(self.path(), 'tendermint')
        if create:
            os.makedirs(p, exist_ok=True)
        return p

    # cache this: the first time, we want to ensure we create the dir,
    # but the rest of the time we just want the string
    @functools.lru_cache(1)
    def dcy_path(self):
        """Return the string pathname of this node's docker-compose."""
        return os.path.join(self.path(create=True), 'docker-compose.yml')

    def dc_str(self):
        """
        Return a string with which sh can run this node in docker-compose.

        Note that this doesn't include the docker-compose command.
        You'll need to append that yourself:

        `node.dc_str() + 'build'`, or `node.dc_str()` + 'run'`, for example.
        """
        docker_compose = os.path.join(project_root(), 'docker-compose.yml')
        return (
            f'NDAUHOME="{self.ndauhome()}" TMHOME="{self.tmhome()}" '
            f'docker-compose -f {docker_compose} -f {self.dcy_path()} '
            f'-p {self.name} --project-directory {project_root()} '
        )


@contextlib.contextmanager
def _generate_script(fn):
    with open(fn, 'w', encoding='utf8', newline='\n') as fp:
        print("#!/bin/sh", file=fp)
        print("# This file was automatically generated by gen_nodes.py.",
              file=fp)
        print("# DO NOT EDIT", file=fp)
        print(file=fp)
        yield fp
    os.chmod(fn, 0o755)  # a=rx;u+w


def build(nodes):
    """Build all nodes using their custom compose variants."""
    for node in nodes:
        print(f'building {node.name}...')
        shell = f'{node.dc_str()} build'
...
# JSG check to see that HONEYCOMB env vars are set
    ...
    chaos_genesis = ndau_genesis = json.loads(ret.stdout)
    ...
    print(f'genesis.json = {chaos_genesis}')

    chaosDir = os.path.join(os.getcwd(), '../', 'helm', 'chaosnode')
    ndauDir = os.path.join(os.getcwd(), '../', 'helm', 'ndaunode')
        ...
        for node in nodes:
            ...
            def chaos_create_peers(peer):
                if (peer.name == node.name):
                    return None
                peerIds.append(peer.chaos_priv['address'])
                return f"{peer.chaos_priv['address']}@{masterIP}:{peer.chaos_p2p_port()}"
            
            peers = list(map(chaos_create_peers, nodes))

            peers = ','.join(list(filter(lambda x: x is not None, peers)))
            peerIds = ','.join(peerIds)
            print(f'peers = {peers}')
            print(f'peerIds = {peerIds}')
            ...
            helm_command = f'helm install --name {node.name} {chaosDir} \
                ...
                --set tendermint.persistentPeers="{b64encode(peers.encode()).decode()}" \
                --set tendermint.privatePeerIds="{b64encode(peerIds.encode()).decode()}" \
            ...
            print(f'chaos shell = {helm_command}')
            ret = run_command(helm_command)
            print(f'chaosnode helm = {ret.stdout}')

            print('Installing {node.name} ndaunode')
            ...
            def ndau_create_peers(peer):
                if (peer.name == node.name):
                    return None
                peerIds.append(peer.ndau_priv['address'])
                return f"{peer.ndau_priv['address']}@{masterIP}:{peer.ndau_p2p_port()}"
            
            peers = list(map(ndau_create_peers, nodes))
            ...
            helm_command = f'helm install --name {node.name} {ndauDir} \
                --set genesis={b64encode(json.dumps(ndau_genesis).encode()).decode()}\
                --set privValidator={b64encode(json.dumps(node.ndau_priv).encode()).decode()}\
                --set nodeKey={b64encode(json.dumps(node.ndau_nodeKey).encode()).decode()}\
                --set tendermint.persistentPeers="{b64encode(peers.encode()).decode()}" \
                --set tendermint.privatePeerIds="{b64encode(peerIds.encode()).decode()}" \
                --set tendermint.nodePorts.enabled=true \
                --set tendermint.nodePorts.p2p={node.ndau_p2p_port()} \
                --set tendermint.nodePorts.rpc={node.ndau_rpc_port()} \
                --set tendermint.moniker={node.name} \
                --set chaosnode.image.tag={NDAU_VERSION} \
                --set tendermint.image.tag={TM_VERSION} \
                --set noms.image.tag={NOMS_VERSION} \
                --set honeycomb.key={HONEYCOMB_KEY} \
                --set honeycomb.dataset={HONEYCOMB_DATASET} \
            ...
            print(f'ndau shell = {helm_command}')
            ret = run_command(helm_command)
            print(f'ndaunode helm = {ret.stdout}')

    except subprocess.CalledProcessError:
        abortClean(f'Could not install with helm: {ret.returncode}')


    print('SUCCESS. Launch with:')
