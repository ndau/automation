...
class Conf:
    ...
    def __init__(self, args):
        ...
        if self.CHAOSNODE_TAG is None:
            try:
                self.CHAOSNODE_TAG = fetch_master_sha(
                    "git@github.com:oneiro-ndev/chaos"
                )
            except OSError as e:
                abortClean(
                    f"CHAOSNODE_TAG env var empty and could not fetch version: {e}"
                )
        ...
        if self.NDAUNODE_TAG is None:
            try:
                ...
                abortClean(
                    f"NDAUNODE_TAG env var empty and could not fetch version: {e}"
                )
...
def main():
    ...
    for node in nodes:
        ...
        chaosLinkOpts = (
            ...
            "--set ndaunode.chaosLink.address="
            f'"{c.MASTER_IP}:{node.chaos["port"]["rpc"]}"'
        )
